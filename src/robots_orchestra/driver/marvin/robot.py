    ...
    try:
        ...
        for line_num, line in enumerate(non_empty_lines, 1):
            ...
            if len(cleaned_values) != expected_columns:
                print(
                    f"第{line_num}行: 期望{expected_columns}列，实际找到{len(cleaned_values)}列"
                )
                return -1
    ...
    result["outputs"] = [
        {
            ...
            "est_joint_firc_dot": [
                round(rt_out.m_EST_Joint_Firc_Dot[j], 4) for j in range(7)
            ],
            "est_joint_force": [
                round(rt_out.m_EST_Joint_Force[j], 4) for j in range(7)
            ],
...
class Marvin_Robot:
    ...
    def get_param(self, type: str, paraName: str):
        ...
        """
        try:
            param_buf = (ctypes.c_char * 30)(
                *paraName.encode("ascii"), 0
            )  # 显式添加终止符
            if type == "float":
    ...
    def get_servo_error_code(self, arm: str):
        ...
        try:
            ...
            if arm == "A":
                self.robot.OnGetServoErr_A.argtypes = [
                ...
                self.robot.OnGetServoErr_B.argtypes = [
                    ctypes.POINTER(ctypes.c_long * 7)
                ]
        ...
        try:
            ...
            if arm == "A":
                return self.robot.OnSetForceCtrPara_A(
                    fc_int, fxDir_arr, fcCtrlPara_arr, adj_double
                )
            elif arm == "B":
    ...
    def set_485_data(self, arm: str, data: bytes, size_int: int, com: int):
        ...
        try:
            # 定义函数原型
            self.robot.OnSetChDataA.argtypes = [
            ...
            # 定义函数原型
            self.robot.OnSetChDataB.argtypes = [
                ctypes.POINTER(ctypes.c_ubyte),
                ctypes.c_long,
                ctypes.c_long,
            ]
            ...
            if (
                result["type"] == "hex string"
                or result["type"] == "bytes"
                or result["type"] == "bytes representation string"
            ):
                print("-" * 50)
                print(f"输入: {data}")
                ...
                print("ERROR: set_485_data input must be hex string of bytes string")
                return False, False
            ...
            if arm == "A":
                return True, self.robot.OnSetChDataA(
                    data_buffer, size_int_long, com_long
                )
            elif arm == "B":
                return True, self.robot.OnSetChDataB(
                    data_buffer, size_int_long, com_long
                )
        except Exception as e:
            print(f"ERROR:{e}")
    ...
    def _send_485_command(
        self, arm: str, data: bytes, channel: int = 2, retry_count: int = 6
    ) -> str:
        """
        通过485通道发送命令并接收响应（内部方法）

        参数:
            arm: 机械手臂ID "A" OR "B"
            data: 要发送的数据字节
            channel: 通信通道，1：'C'端; 2：com1; 3:com2，默认2
            retry_count: 重试次数，默认6次

        返回:
            str: 十六进制字符串格式的响应数据，失败返回空字符串
        """
        init = self.clear_485_cache(arm)
        if not init:
            print(f"failed:清除通道{arm}数据失败!")
            return ""

        time.sleep(0.1)
        send_res = self.set_485_data(arm, data, len(data), channel)
        if not send_res[1]:
            print(f"failed:发送通道{arm}数据失败!")
            return ""

        time.sleep(0.1)
        for i in range(retry_count):
            res, recv_data = self.get_485_data(arm, channel)
            if res > 0:
                return recv_data
            time.sleep(0.1)
        return ""
    ...
    # ========== 夹爪控制相关方法 ==========
    # 注意：推荐使用 gripper_control.py 中的 GripperController 类
    # 以下方法保留用于向后兼容，内部使用 Modbus-RTU 协议

    def get_gripper_controller(
        self, arm: str = "A", device_id: int = 1, channel: int = 2
    ):
        """
        获取夹爪控制器实例

        参数:
            arm: 机械手臂ID "A" OR "B"，默认'A'
            device_id: Modbus设备ID，默认1
            channel: 通信通道，1：'C'端; 2：com1; 3:com2，默认2

        返回:
            GripperController: 夹爪控制器实例
        """
        from .gripper import GripperController

        def send_callback(command: bytes):
            """
            发送命令回调函数
            ...
            """
            # 清除缓存
            if not self.clear_485_cache(arm):
                print(f"failed:清除通道{arm}数据失败!")
                return None
            ...
            # 发送命令
            send_res = self.set_485_data(arm, command, len(command), channel)
            if not send_res[1]:
                print(f"failed:发送通道{arm}数据失败!")
                return None
            ...
            # 接收响应
            for i in range(6):
                res, recv_hex = self.get_485_data(arm, channel)
                if res > 0:
                    # 将十六进制字符串转换为字节
                    recv_data = bytes.fromhex(recv_hex.replace(" ", ""))
                    return recv_data
                time.sleep(0.1)

            return None

        return GripperController(send_callback, device_id)

    def init_gripper(
        self, arm: str = "A", is_full: bool = False, channel: int = 2
    ) -> bool:
        """初始化夹爪
        ...
        参数:
            ...
            is_full: False表示单方向初始化（寻找最大位置或最小位置），
                    True表示进行一次张开闭合初始化，默认False
            channel: 通信通道，1：'C'端; 2：com1; 3:com2，默认2

        返回:
            bool: True表示成功，False表示失败
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.init(is_full=is_full)

    def set_gripper_force(self, arm: str, value: int, channel: int = 2) -> bool:
        """设置夹爪力值
        ...
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.set_force(value)

    def set_gripper_position(self, arm: str, value: int, channel: int = 2) -> bool:
        """设置夹爪目标位置
        ...
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.set_target_position(value)

    def get_gripper_force(self, arm: str, channel: int = 2) -> int:
        """获取夹爪力值
        ...
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.get_force()

    def get_gripper_position(self, arm: str, channel: int = 2) -> int:
        """获取夹爪目标位置
        ...
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.get_target_position()

    def get_gripper_init_state(self, arm: str, channel: int = 2) -> int:
        """获取夹爪初始化状态
        ...
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.get_init_state()

    def get_gripper_hold_state(self, arm: str, channel: int = 2) -> int:
        """获取夹爪夹持状态
        ...
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.get_hold_state()

    def get_gripper_current_position(self, arm: str, channel: int = 2) -> int:
        """获取夹爪当前位置
        ...
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.get_current_position()

    def get_gripper_current(self, arm: str, channel: int = 2) -> int:
        """获取夹爪电流
        ...
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.get_current()

    def save_gripper_para(self, arm: str, channel: int = 2) -> bool:
        """保存夹爪参数
        ...
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.save_para()

    def set_gripper_init_mode(self, arm: str, value: str, channel: int = 2) -> bool:
        """设置夹爪初始化方向
        初始化结束后停留方向
        已添加写入保存，不要重复操作
        ...
        """
        gripper = self.get_gripper_controller(arm, channel=channel)
        return gripper.set_init_mode(value)
    ...
    def identify_tool_dyn(self, robot_type: int, ipath: str):
        ...
        :return:
            m,mcp,i
        """
        if not isinstance(robot_type, int):
        ...
        success1 = self.robot.FX_Robot_Iden_LoadDyn(
            robot_type_, path_char, mm_ptr, mcp_ptr, ii_ptr
        )
...
class RT_IN(Structure):
    _fields_ = [
        (
            "m_RtInSwitch",
            c_int,
        ),  # * 实时输入开关 用户实时数据 进行开关设置 0 -  close rt_in ;1- open rt_in*
        ...
        (
            "m_Pvt_RunState",
            c_ubyte,
        ),  # 0: idle空闲; 1: loading正在加载 ; 2: running正在运行; 3: error出错啦
    ]
...
class DCSS(Structure):
    _fields_ = [
        ...
        (
            "m_ParaRetSerial",
            c_short,
        ),  # short working: 0; finish: cmd serial; error cmd_serial + 100
    ]
